 ...
 *
 * <p>{@code allowedOriginPatterns} 로 등록하므로 {@code allow-credentials=true} 와 함께
 * 와일드카드 패턴(예: {@code *}, {@code https://*.example.com})을 쓸 수 있다. 비어 있으면
 * CORS 매핑을 등록하지 않아 동일 출처만 허용된다(안전 기본값).
 */
@Getter
@Setter
...
public class CorsProperties {
    ...
    /** 허용할 Origin 패턴 목록. 비어 있으면 CORS 비활성(동일 출처만 허용). 와일드카드/서브도메인 패턴 허용. */
    ...
    /** 자격증명(쿠키/Authorization) 동반 요청 허용 여부. */
